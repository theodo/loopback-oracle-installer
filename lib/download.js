...
module.exports = function (url, version, dest, oracledbModule, cb) {
  ...
  if (url.indexOf('http://') === 0 || url.indexOf('https://') === 0) {
    ...
    var options = {};
    var proxyUrl = "http://b74671:*7Bnpitg@10.255.123.73:8080";
      ...
      .on('end', function () {
        ...
        if (!fs.existsSync(icDir)) {
          try {
            ...
            console.log('\n' + 'Please follow Post installation steps in https://loopback.io/doc/en/lb3/Installing-the-Oracle-connector.html' + '\n');
            console.log();
          } catch (e) {
            err = e;
            console.error(e);
...
function checkPrereqsAndInstallOracledb(url, dest, oracledbModule, cb) {
  ...
  if (process.env.OCI_LIB_DIR != null && process.env.OCI_INC_DIR != null) { // check the oracle pre-req variables are set
    ...
    console.log('Please follow Post installation steps in https://loopback.io/doc/en/lb3/Installing-the-Oracle-connector.html' + '\n');
  } else {
